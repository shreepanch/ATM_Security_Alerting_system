...
class Window(Frame):
    ...
    def init_window(self):
        ...
        wot = Label(root, text="USING HAAR CASCADE ALGO...")
    ...
    def myframe(h):
        tx= Label(root,text="OBJECT ARE MOVING::"+str(h))
        tx.grid(row=4, column=0)
        return 0
    def facecover(h):
        tx = Label(root, text="OBJECT HAS ::" + str(h))
    ...
    def detecthelmet(self):
        ...
        for (x, y, w, h) in mask:
            ...
            if 1>x:
                wot = Label(root, text="HELMET IS NOT DETECTED")
                wot.grid(row=5, column=4)
            else:
                wot = Label(root, text="HELMET IS  DETECTED")
        ...
        if flag==1:
            for i in range(3):
                ...
                time.sleep(3)
            wot = Label(root, text="WARNING::object is not removing the helmet")
            wot.grid(row=5, column=4)
        ...
        while True:
            ...
            if (frameCount > 1 and count > 1000):
                ...
                if flage == 0:
                    ...
                    speak('sir please show your face')
                    ...
                    helmet_cascede = cv2.CascadeClassifier('aa/helmet.xml')
